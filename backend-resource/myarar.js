...
function ARARTree ( inputData, segmentMode=false, useExchange=true, initialLevel=1 ) {
	let createTime, traversalTime, memoryUsed;
	let ruleList, treeRoot, node, leafList = [];
	let queue = new QueueObject(3, Math.pow(2,32)-1, true);

	// convert the ruleList
	ruleList = inputDataConvertor(inputData);
	// console.log(ruleList[ruleList.length-1]);

	// if need to exchange
	if ( useExchange ) {
		ruleList = exchangeRuleList(ruleList);
		// console.log(ruleList[ruleList.length-2]);
		// console.log(ruleList[ruleList.length-1]);
	}
	...
	while ( true ) {
		...
		if ( node ) {
			let childSum = 0;
			// console.log(ipConvertor(node['parameter']['rsvSrc']), ipConvertor(node['parameter']['rsvDest']));
			if ( node['node00'] ) {
				queue.push(node['node00']);
				childSum++;
			}
			if ( node['node01'] ) {
				queue.push(node['node01']);
				childSum++;
			}
			if ( node['node10'] ) {
				queue.push(node['node10']);
				childSum++;
			}
			if ( node['node11'] ) {
				queue.push(node['node11']);
				childSum++;
			}
			// if (node['parameter']['segTimes'] === 3) break;
