/* Shield UI 1.7.29 Trial Version | Copyright 2013-2017 Shield UI Ltd. | http://www.shieldui.com/eula */ ! function(a, b, c, d) {
    ...
    e = {
        ...
        checkboxes: { enabled: !1, children: !1, template: d },
        dragDrop: !1,
        dragDropScope: d,
        textTemplate: "{text}",
        hrefTemplate: "{href}",
                ...
                f = function(b) {
                    ...
                    return a(b).children(P).each(function() {
                        var b, d = a(this),
                            ...
                            g = { cls: d.attr("data-class"), href: d.attr("data-href") ? d.attr("data-href") : d.children().not(Q).first().attr("href"), disabled: d.attr("data-disabled") ? d.attr("data-disabled") : q(d.attr(H)) && null !== d.attr(H), expanded: d.attr("data-expanded"), checked: d.attr("data-checked"), iconUrl: d.attr("data-icon-url"), iconCls: d.attr("data-icon-cls")};
                        e ? (b = d.clone(), b.children(Q).remove(), g.text = b.html()) : g.text = d.html(), e && (g.items = f(d.children(Q)[0])), c.push(g)
                    }), c
                }, j.dataSource = h.create({ data: b, schema: { parse: f } })
            }
            j.dataSource.on(D + i, m(j._dsChange, j)), k.attr(x, "tree"), l.readDataSource && j.dataSource.read()
        },
        refresh: function(a) { this.refreshWithElement(this._original, a) },
        _dsChange: function() {
            var a = this,
                b = a.element,
                c = a.dataSource;
            a._renderItems(c, b, null, !0, !0, function() { a.trigger("dataBound", { dataSource: c, element: null }) })
        },
        _destroyItems: function(b) {
            var c = this;
            a(b).children("ul.sui-treeview-item-list").each(function() { c._destroyItems(a(this)) }), a(b).children(".sui-treeview-item-content").find(".sui-treeview-item-text").each(function() { a(this).swidget("Draggable") && a(this).swidget("Draggable").destroy(), a(this).swidget("Droppable") && a(this).swidget("Droppable").destroy() }), a(b).children(".sui-treeview-item-content").find(".sui-checkbox").each(function() { a(this).swidget("CheckBox") && a(this).swidget("CheckBox").destroy() });
            var d = a(b).data(Y);
        ...
        _renderItem: function(c, e, f, g) {
			var $content = a('<div class="sui-treeview-item-content"/>'); // Ren-Wei added for customProperty.
            ...
            if (h = p(G.disabledTemplate, c), h = !!(r(h) && h || "true" === h || "1" === h || 1 === h), i = p(G.expandedTemplate, c), i = !!(r(i) && i || "true" === i || "1" === i || 1 === i), j = p(G.checkedTemplate, c), j = !!(r(j) && j || "true" === j || "1" === j || 1 === j), n = p(G.hrefTemplate, c), v(n) && "undefined" !== n || (n = d), o = p(G.iconUrlTemplate, c), v(o) && "undefined" !== o || (o = d), t = p(G.iconClsTemplate, c), v(t) && "undefined" !== t || (t = d), u = a('<li class="' + ea + " sui-unselectable" + (h ? " " + da : "") + (q(O) ? " " + O : "") + '"/>').data(U, c).data(V, g).data(W, e).data(X, f), D = a('<span class="sui-treeview-item-text" id="' + T + '">' + (n ? '<a href="' + n + '">' : "") + Q + (n ? "</a>" : "") + "</span>").on(F, m(E._itemClick, E, u)).on(A, m(E._itemDblClick, E, u)), w = a('<span class="sui-treeview-item-toggle"/>').on(z, m(E._toggleItem, E, u)).append(a('<span class="sui-treeview-item-toggle-collapsed"/>')), P || w.css(R, S), u.attr(x, "treeitem").attr("aria-describedby", T), P && u.attr(y, J), H.enabled)
                if (u.data(Z, !!j), B = a('<span class="sui-treeview-item-check"/>'), v(I)) B.html(p(I, { item: c, parent: N }));
                else if (s(I)) I(B, c, N);
            else {
                var Y = a('<input type="checkbox"' + (j ? ' checked="checked"' : "") + "/>").appendTo(B);
                b.ui.CheckBox ? a(Y).shieldCheckBox({ enabled: !h, enableThreeStates: !!H.children, events: { click: m(E._checkboxClick, E, u) } }) : Y.on(z, m(E._checkboxClick, E, u))
            }
			
			// Ren-Wei added for customProperty.
			if ( c['customProperty'] ) { 
				for ( var count = 0; count < c['customProperty'].length; count++) {
					var	name = c['customProperty'][count]['name'],
						value = c['customProperty'][count]['value'];
					$content.attr(name, value || "");
				}
			}

            return (o || t) && (C = a('<span class="sui-treeview-item-icon"/>'), o ? C.append('<img src="' + o + '"/>') : C.addClass(t)), u.append($content.append(w, B, D.prepend(C))), u.append(a('<ul class="sui-treeview-list sui-treeview-item-list"/>').attr(x, "group").hide()), G.dragDrop && !h && (D.shieldDraggable({ scope: q(L) ? L : "suiTv" + K, dragCls: "sui-treeview-item-text-dragging", helper: M, events: { start: m(E._dragStart, E, u), stop: m(E._dragStop, E, u) } }), D.shieldDroppable({ scope: q(L) ? L : "suiTv" + K, hoverCls: "sui-treeview-item-text-dropover", tolerance: { x: "touch", y: "intersect" }, events: { over: m(E._dropOver, E, u), out: m(E._dropOut, E, u), drop: m(E._drop, E, u) } })), { listItem: u, itemExpanded: i, itemChecked: j, itemDisabled: h }
        },
        ...
        _dragStart: function(a, b) { this.trigger("dragStart", { draggable: b.target.element, element: a, domEvent: b }) },
        _dragStop: function(a, b) { this.trigger("dragStop", { draggable: b.target.element, element: a, domEvent: b }), b.preventDefault() },
        _dropOver: function(b, c) {
            var d, e = c.draggable,
                f = c.droppable,
                g = this._isValidDragDrop(e, f),
                h = { draggable: e, droppable: f, element: b, valid: g, domEvent: c };
        ...
        _drop: function(b, c) {
            var d, e = this,
                ...
                i = { draggable: f, droppable: g, element: b, cancelled: !0, skipAnimation: !!h, valid: h, domEvent: c };
        ...
        _blur: function(b) {
            var c = this;
            c._isFocused() && (c._blurTimeout && clearTimeout(c._blurTimeout), c._blurTimeout = setTimeout(function() { a(c.element).removeClass("sui-treeview-focus"), c.trigger(C) }, 100))
        },
        _selectItem: function(a, b, c) {
            var d, e = this;
            e._isSelected(a) || c && (d = e.trigger("select", { element: a, item: a.data(U) }), d.isDefaultPrevented()) || (e._selectedItem && e._selectedItem.removeClass(ba), e._selectedItem = a, a.addClass(ba), b && e._activateItem(a, c))
        },
        _activateItem: function(a, b) {
            var c = this;
            c._isActive(a) || (c._activeItem && c._activeItem.removeClass(ca), c._activeItem = a, a.addClass(ca), b && c.trigger(D, { element: a, item: a.data(U) }))
        },
        ...
        _toggleCheck: function(a, b) { this._setChecked(a, !a.data(Z), b) },
        _setChecked: function(b, c, d) {
            var e = this,
                f = e.options.checkboxes.children,
                g = e._getCheckBox(b);
            g && g.length > 0 && (a(g).swidget("CheckBox") ? (g = a(g).swidget("CheckBox"), g.checked(c)) : c ? g.attr(O, O) : g.removeAttr(O), b.data(Z, c), f && e._updateChecked(b), d && e.trigger("check", { element: b, item: b.data(U), checked: c }))
        },
        ...
        _expand: function(b, c, d, e) {
            ...
            if (k && !(a(k).length <= 0) && (i.hasChildren(b) || !(k.children(P).length <= 0))) return i._isExpanded(b) ? void(s(e) && e.call(i)) : void(c && (h = i.trigger("expand", { element: b, item: b.data(U) }), h.isDefaultPrevented()) || (k.stop(!0, !0), k.hide(), f = i._getToggleEl(b), g = function() { k.slideDown(j.enabled && !d ? j.expandDuration : 0, function() { b.addClass(_).attr(y, I), f.css(R, T), a(f).find("span").addClass("sui-treeview-item-toggle-expanded").removeClass("sui-treeview-item-toggle-collapsed"), s(e) && e.call(i) }) }, i._isLoaded(b) ? g() : i._loadItem(b, d, g)))
        },
        _loadItem: function(b, c, d) {
            ...
            !j || a(j).length <= 0 || (t(m) || u(m) || m instanceof h ? (a(l).addClass(aa), e = h.create(m), j.data(Y, e), e.on(D + f._eventNS, function(g) { a(l).removeClass(aa), b.data($, !0), f._renderItems(this, j, b, !0, c, function() { f.trigger("dataBound", { dataSource: e, element: b }), s(d) && d.call(f) }) }), e.read({ parent: g })) : k && k instanceof i ? (a(l).addClass(aa), e = new i(k.options), j.data(Y, e), e.on(D + f._eventNS, function(g) { a(l).removeClass(aa), b.data($, !0), f._renderItems(this, j, b, !0, c, function() { f.trigger("dataBound", { dataSource: e, element: b }), s(d) && d.call(f) }) }), e.read({ parent: g })) : (b.data($, !0), s(d) && d.call(f)))
        },
        _expandPathSegment: function(a, b, c, d, e) {
            var f, g, h = this,
                i = a.length;
            return d > i ? void(s(e) && e.call(h)) : (f = [].slice.call(a).splice(0, d), g = h.getByPath(f), void(h._isExpanded(g) ? h._expandPathSegment(a, b, c, d + 1, e) : h._expand(g, b, c, function() { h._expandPathSegment(a, b, c, d + 1, e) })))
        },
        _expandPath: function(a, b, c, d) { this._expandPathSegment(a, b, c, 1, d) },
        _collapse: function(b, c) {
            var d, e, f = this,
                g = f.options.animation,
                h = b.children("ul.sui-treeview-item-list").first();
            !f._isExpanded(b) || !h || a(h).length <= 0 || c && (e = f.trigger("collapse", { element: b, item: b.data(U) }), e.isDefaultPrevented()) || (d = f._getToggleEl(b), h.stop(!0, !0), h.slideUp(g.enabled ? g.collapseDuration : 0, function() { b.removeClass(_).attr(y, J), a(d).find("span").removeClass("sui-treeview-item-toggle-expanded").addClass("sui-treeview-item-toggle-collapsed") }))
        },
        _keydown: function(a) {
            var b = this,
                c = !0;
        ...
        updateIndetermined: function(b) {
            var c = this,
                d = c._getListItem(b);
            q(d) ? c._updateChecked(a(d)) : a(c.element).children(P).each(function() { c._updateChecked(a(this)) })
        },
        expanded: function() {
            var a = this,
        ...
        append: function() {
            ...
            b = e._insertedItem(g, c), b && (h && e._beforeRemoveChild(e.getParent(b)), !c || e._isExpanded(c) ? (b.appendTo(d), e._refreshParent(b), s(j) && j.call(e, b)) : e.hasChildren(c) ? e._isExpanded(c) ? (b.appendTo(d), e._refreshParent(b), s(j) && j.call(e, b)) : e._expand(c, !1, !1, function() { b.appendTo(d), e._refreshParent(b), s(j) && j.call(e, b) }) : e._isExpanded(c) ? (b.appendTo(d), e._refreshParent(b), s(j) && j.call(e, b)) : (a(e._getToggleEl(c)).css(R, T), c.removeClass(_), b.appendTo(d), e._refreshParent(b), e._expand(c, !1, !1, function() { s(j) && j.call(e, b) })))
        },
